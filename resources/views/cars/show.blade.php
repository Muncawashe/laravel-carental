...
<div class="flex p-4 mt-8">
    ...
    <div class="flex w-1/2 ">
        <div class="p-2 rounded-md flex flex-col">
            ...
            <div>
                <button onclick="showForm()" class="ml-4 mb-0 bg-transparent hover:bg-green-500 text-green-700 font-semibold hover:text-white py-2 px-4 border border-green-500 hover:border-transparent rounded">Book Now</button>
            </div>
        </div>
    </div>
</div>

<div id="dialogue" class="fixed left-0 top-20 bg-black bg-opacity-75 w-screen h-screen items-center hidden">
    ...
    <form class="grid grid-cols-4 gap-4 form bg-white p-6 border-1 rounded-xl mt-12" method="POST" enctype="multipart/form-data"
        ...
        <div class="">
            ...
            <input class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  w-full ps-10 p-2.5" type="text" id="make"  name="make" value="{{ $car['make'] }}">
            @error('make')
        ...
        <div>
            ...
            <input class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  w-full ps-10 p-2.5" type="text" id="model" name="model" value="{{ $car['model'] }}">
            @error('model')
                <div class="form-error">
                    {{ $message }}
                </div>
        ...
        <div class="flex">
            <button class="w-full ml-4 mb-0 bg-transparent hover:bg-green-500 text-green-700 font-semibold hover:text-white py-2 px-4 border border-green-500 hover:border-transparent rounded" type="submit">Book Vehicle</button>
        </div>
        <div class="flex">
            <button onclick="hideForm()" class="w-full ml-4 mb-0 bg-transparent hover:bg-red-500 text-red-700 font-semibold hover:text-white py-2 px-4 border border-red-500 hover:border-transparent rounded" type="submit">Cancel Order</button>
        </div>
    </form>
...
<script>
    function showForm(){
        let form = document.getElementById("dialogue");
